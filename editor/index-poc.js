function insertStyle(selector, src) {
...
function insertScript(selector, type, src, defer, async) {
...
function runScripts() {
  if (parent.frames.length === 0) {
      ...
      const adminHtml = `
        <div class="layout">
          ...
          <main class="main bg-neutral-100">
            <iframe class="iframe js-iframe transition-[width] shadow-lg" name="klar-app" src="/"></iframe>
          </main>
        </div>
      `;
    ...
    function setEvents() {
      ...
      setTimeout(()=>desktop.click(), 200);
    }
    addHtml();
    insertScript('head', null, 'https://cdn.tailwindcss.com', null, true);
    insertStyle('head', '/admin/index.css');
    setEvents();
  } else {
    function addRootHtml() {
      const adminHtml = `<div id="root"></div>`;
      document.body.append(new DOMParser().parseFromString(adminHtml, 'text/html').getElementById('root'));
    }
    function insertBabelScripts() {
      insertStyle('head', 'http://localhost:4173/assets/index.9d202c10.css');
      insertScript('body', 'text/javascript', 'https://unpkg.com/react@18/umd/react.production.min.js');
      insertScript('body', 'text/javascript', 'https://unpkg.com/react-dom@18/umd/react-dom.production.min.js');
      insertScript('body', 'text/javascript', 'https://cdnjs.cloudflare.com/ajax/libs/babel-standalone/7.20.2/babel.min.js');
      insertScript('body', 'text/babel', 'blocks/hero.js');
      insertScript('body', 'text/babel', 'blocks/team.js');
      insertScript('body', 'text/babel', 'blocks/index.js');
      insertScript('body', 'text/javascript', 'https://mozilla.github.io/nunjucks/files/nunjucks.min.js');
    }
    addRootHtml();
    insertBabelScripts();
  }
